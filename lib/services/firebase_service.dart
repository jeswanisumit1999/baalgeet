...
class FirebaseService {
  ...
  //     });
  //   } else {
  //     var days = course.courseMeta.duration;
  ...
  //       'course_expiry_date': DateTime.now().add(const Duration(days: int.parse(days!))),
  //     }},
  // });
  //   }
    if (courses.contains(newCourseId)) {
      ...
      var days = course.courseMeta.duration;
      int durationDays = int.parse(days!); // Ensure days is not null and convert it to an integer

      // Add the new course to the list of courses
      courses.add(newCourseId);

      // Update the 'enrolled' field to include the new course
      await ref.update({
        'enrolled': FieldValue.arrayUnion(courses),
      });

      // Calculate the course expiry date and update the database
