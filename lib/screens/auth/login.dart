...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  void afterSignIn() async {
    if (widget.popUpScreen == null || widget.popUpScreen == false) {
      ...
      print("In afterSignIn User Id : " + user!.uid.toString());
      try {
        deviceId = await _getId();
        // Get the Firestore reference to the user document
        DocumentReference userDocRef = FirebaseFirestore.instance.collection('users').doc(user?.uid);

        // Update the user's document with the device ID
        await userDocRef.set({
          'deviceId': deviceId,  // Store the device ID in Firestore
        }, SetOptions(merge: true));

        print("Device ID successfully updated in Firestore");
      } catch (e) {
